...
# distribute kubelet, kube-proxy configuration files to nodes
for instance in node-0 node-1 node-2; do
    scp -F config\
        ${instance}.kubeconfig kube-proxy.kubeconfig ${instance}:~/
done

# distribute admin, kube-controller-manager, kube-scheduler configuration files to masters
